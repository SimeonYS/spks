...
class SpksPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('spks.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `spks`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM spks WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO spks (date, title, link, content)"
